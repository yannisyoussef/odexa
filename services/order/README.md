...
- JWT `tenant_id` and `sub` are the only tenant/customer authority. Both endpoints require `CUSTOMER`; other roles do not bypass ownership. Reads filter tenant and owner together and return 404 for all absent/inaccessible IDs.
...
The listener's inbox insert, row lock, state/evidence mutation and confirmation outbox append share one database transaction. Duplicates are inert. Unknown event types/versions, malformed owned payloads, missing tenant orders, or conflicting preterminal evidence throw sanitized errors for runtime bounded recovery/DLT; their inbox inserts roll back. A terminal order never reopens or changes outcome. Runtime event access must be restricted to trusted service producers; the envelope is not a public callback authentication mechanism.
...
- `OrderStateMachineTest`: all 20 state/signal pairs, deferred causality, contradictory outcomes and immutability.
